...
- uses the same number of history frames;
- is rescored by the current Cube;
- applies the same fixed duplicate suppression;
...
static/dynamic and near/far slices, and the fraction of exported points supplied
by history. Scene-first uncertainty is mandatory because the validation cohort
is small.
