...
| C9 | Separating occupancy training queries from radar-proposal inference queries reduces outliers while retaining fixed-count dense geometry. | G1D: 6.25% occupied and range-stratified reliable-empty queries; 32k coarse radar proposals; 2.5k selection; 10k local refinement; absolute and paired control gates over three seeds. | Protocol frozen before any G1C metric was observed; implementation and runs pending. | New independent gate | “We test coarse-to-refine radar query fields as an independently named alternative to failed occupancy ranking.” |
...
| G1D | `docs/g1d_rald_query_field_protocol.md`; `docs/g1d_rald_source_map.md`; `artifacts/g1/g1d_preflight_d0c8c6fb.json`; server root `formal_d0c8c6f_g1d_rald_query_field` | Cache-equivalent H200 preflight passed at source `d0c8c6fb`; Stage A seed 20260716 is running; no scientific gate result yet |
