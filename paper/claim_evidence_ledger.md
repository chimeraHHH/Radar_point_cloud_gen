...
| C8 | RaLD's central latent-diffusion mechanism can model dense physical point states when query allocation is independently validated. | G1E/G3L: source-faithful `512 x 32` target posterior, coordinate-only implicit decoder, 24-layer Full-RAED-conditioned EDM, official 18-step sampler, condition-shuffle control, and no best-of-k. | The earlier anchor G3L implementation is structurally verified but empirically locked. Before the G1D result, G1E-D0 was frozen to test whether deterministic Full-RAED proposal support rescues the archived latent-only VAE checkpoints; no G1E result exists yet. | New independent gate | “We preregister a two-stage RaLD latent-generation candidate; no performance claim is made.” |
...
| G1E | `docs/g1e_rald_latent_edm_protocol.md`; archived R1 checkpoints and gate records under `artifacts/baselines/rald/` | D0 was frozen before the G1D Stage-A result. It reuses the existing R1 weights and changes only inference query support; VAE/EDM training remains unauthorized until D0 passes. |
