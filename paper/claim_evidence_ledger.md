...
| C9 | Separating occupancy training queries from radar-proposal inference queries reduces outliers while retaining fixed-count dense geometry. | G1D: 6.25% occupied and range-stratified reliable-empty queries; RaLD-equivalent all-query BCE; 32k coarse radar proposals; 2.5k selection; 10k local refinement; absolute and paired control gates over three seeds. | Corrected implementation must pass a new source-bound preflight and Stage A. The invalid `d0c8c6f` run is audit-only and cannot support the claim. | New independent gate | “We test coarse-to-refine radar query fields as an independently named alternative to failed occupancy ranking.” |
...
| G1D | `docs/g1d_rald_query_field_protocol.md`; `docs/g1d_rald_source_map.md`; `artifacts/g1/g1d_invalid_d0c8c6f_loss_normalization.json`; historical preflight `artifacts/g1/g1d_preflight_d0c8c6fb.json` | `d0c8c6fb` Stage A stopped at epoch 7 and invalidated because its classwise weighted BCE was not RaLD-equivalent. Corrected all-query BCE requires a new preflight and Stage A; no scientific gate result yet. |
