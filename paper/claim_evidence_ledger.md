...
| C12 | Wide continuous spatial queries or Doppler-warped history provide support that is absent from the frozen 32k current-frame pool. | Corrected G1T temporal support; R-A1 wide-query Stage-0; no learned selector claim; complete far-target frame accounting. | Corrected G1T was no-go. R-A1 completed 20 epochs and achieved completeness `1.5081 m`, far completeness `11.1211 m`, and wrong-condition degradation `12.987%`, but failed mean Chamfer (`4.0090 m`), outlier (`31.8129%`), and matched-win (`66.67%`) gates. | Mixed mechanism evidence; method no-go | “Wide queries improved coverage and learned Cube dependence, but the fixed-10k output failed precision and outlier gates.” |
