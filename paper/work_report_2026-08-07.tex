...
后继路线退休；exact 10k 与真实 5 cm 间距保留。当前已冻结 Q-Local-F0R
全域导出容量协议，但尚无该路线的 H200 结果。逐点 Doppler、Cube-cycle、
时序生成和测试集评估仍未解锁。
\end{minipage}}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{最窄后继 Q-Local-F0R 已冻结，当前仍是零训练待执行门。}
  F0R 保持 Fresh-WCE-20、700k 坐标、Q0/Q1、residual、base confidence、
  Q-Local scorer 和 12 帧 cohort 不变，只把三段硬配额改为全域稳定排序加
  5 cm greedy selection。它仍须逐帧通过 $\mathrm{CD}\le0.8$ m、outlier
  $\le5\%$、exact 10k，并不得牺牲任何含 target 的中远距区间。
  只有该同场 capacity oracle 通过，才允许另行冻结 500-update scorer 训练；
  若失败，才进入 variable multi-return ray-hazard 表示。
...
Q-Local-F0R & \pending & source-bound 零训练协议已冻结；候选、scorer、cohort、exact 10k 与 5 cm 均保持 & 只取消硬 range quota；H200 capacity 结果尚未形成 \\
...
Q-Local-F0R 在本报告截点只有冻结协议与待验证实现，不登记 checkpoint、训练 step
或 H200 科学终态。
...
Q-Local-F0R exporter & 与 Q-Local-F0 bit-identical 的 700k candidates 与 score；全域稳定排序 + 5 cm greedy selection & exact 10k，不设逐帧 range quota；含 target 区间保留门 & 协议已冻结；实现待干净 H200 回归 \\
...
\end{equation}
当前协议已冻结，预检实现处于待回归状态，H200 科学结果尚未形成。若 12/12 帧
全部通过，只授权另行
冻结的 500-update Q-Local 训练；若任一科学门失败，才转向 variable multi-return
renewal-hazard field。固定 $K=4/6$ measured-peak 与旧硬配额均不得重开。
...
76-frame fixed-quota feasibility audit 给出两个严格反例，故硬配额已退休。
最窄后继 Q-Local-F0R 保持 Fresh-WCE、700k coordinates、Q0/Q1 与 Q-Local
scorer 全部不变，只另行冻结无硬 range quota 的全域 exact-10k exporter capacity
oracle；只有该 F0R oracle 仍失败，才更换为 variable multi-return ray-hazard。
任何后继都不能删除失败帧、改成 aggregate-only gate 或事后放宽几何门。
...
F0R & global-export candidate oracle & 当前待执行，零训练 & 协议已冻结；保持 700k field，只取消逐帧硬配额 \\
...
又在训练前的容量门停止。固定 range quota 审计已经证明旧输出契约不可复用，
因此目前没有活动的 learned fixed-10k 几何训练；当前活动路线是只改变 exporter
的 Q-Local-F0R 零训练 capacity oracle，ray-hazard 仍是其失败后的后备。
...
因此当前最优先工作不再是增加网络深度，而是执行已经冻结的 Q-Local-F0R
global-export capacity oracle；只有同场 700k candidate field 仍失败，才有证据
支持切换到 variable multi-return renewal-hazard successor。
...
固定配额报告 SHA &
\hash{d5d9eeaae99a97bb8d358ce2de7c6c48}{92e221e82b1e64eaf2897aa038cb627c} \\
Q-Local-F0R 状态 & 零训练协议已冻结；H200 capacity 结果尚未形成 \\
RaLD 审计 commit &
...
\begin{itemize}[itemsep=0.15em,topsep=0.2em]
  ...
  \item 本报告没有把 Q-Local capacity no-go 泛化成所有 continuous quality scorer 失败；严格 fixed-range 审计只否决旧逐帧硬配额。
  \item 本报告没有把 Q-Local-F0R 的冻结协议或未归档实现写成 H200 科学结果。
