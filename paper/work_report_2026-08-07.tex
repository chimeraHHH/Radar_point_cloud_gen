...
\hypersetup{
  ...
  pdftitle={4D Radar Cube to Dense Point Cloud: Work Report 2026-08-08}
}
...
\fancyhead[R]{工作报告更新至 2026-08-08}
\fancyfoot[C]{\thepage}
...
\textbf{2026-08-08 STDA-F0 协议冻结完成时}。文中严格区分“已实现”“已验证”、
“已跳过”和“仅计划”：
...
固定 $8000/1700/300$ 距离配额与 $0.8$ m Chamfer/5\% outlier 门存在模型无关的
矛盾。随后完成的 76-frame train-only 严格审计在 47:94 与 58:404 上给出
不可行下界，终态为 \code{fixed\_range\_quota\_contract\_no\_go}。旧硬配额已从
后继路线退休；exact 10k 与真实 5 cm 间距保留。随后 source
\code{34579a2} 在物理 H200 GPU0 完成 Q-Local-F0R：全域导出在 12/12 帧通过
CD、outlier 与结构门，但只有 2/12 保留全部 target-bearing range strata，终态为
\code{qlocal\_f0r\_capacity\_no\_go}。随后 source \code{380f3ea} 在同一物理
H200 GPU0 完成 VRH-F0 12-frame paired capacity oracle：实现、资源与 renewal
activity 有效，但 sequential-frontier 的 mean CD 为 0.5494 m，差于同流 flat
control 的 0.1095 m；strata retention 为 3/12 vs 4/12，first/later 完整门均为
0/12。终态 \code{vrh\_f0\_capacity\_no\_go}，Q-Local 与 learned renewal 均未训练。
后继 STDA-F0 经首轮四位独立审计者和六轮逐字修订审计，已对协议 SHA-256
\code{a1bacd61\ldots edf84} 获得 \code{FREEZE} 并完成外部 freeze record；
这只授权 all-76 零训练容量 oracle 的实现，不构成科学结果或 learned 方法授权。
G4 数据 full-summary CRC 已通过，但逐点 Doppler、
...
\begin{enumerate}
  ...
  \item \textbf{固定逐帧 range quota 已被严格否决，不能再由后继路线继承。}
  对 76/76 个 train 帧执行只读取 target range 的 reverse-triangle-inequality
  审计后，47:94 与 58:404 的乐观 Chamfer 下界分别为 $0.8210$ m 与
  $1.3245$ m，已经超过 $0.8$ m；58:404 还被强制产生至少 20\% 的 2 m
  离群点。该结论与网络、候选场、角度误差、5 cm packing 和 completeness
  无关。旧 $8000/1700/300$ 配额退休，但 exact 10k 和真实 5 cm 间距保留。

  \item \textbf{Q-Local-F0R 已科学 no-go，失败定位到集合级距离分配。}
  F0R 保持 Fresh-WCE-20、700k 坐标、Q0/Q1、residual、base confidence、
  Q-Local scorer 和 12 帧 cohort 不变，只把三段硬配额改为全域稳定排序加
  5 cm greedy selection。不可部署 GT-nearest oracle 在 12/12 帧通过
  $\mathrm{CD}\le0.8$ m、outlier $\le5\%$、exact 10k 与结构门，但只有
  2/12 保留所有含 target 的距离段。10 帧把预算集中到近距，故 500-update
  scorer 训练继续禁止；该结果只授权零训练的 VRH-F0 容量门。

  \item \textbf{VRH-F0 已完成，续接活动成立但续接效用不成立。}
  source \code{380f3ea} 在 H200 GPU0 完成 12 帧同流 paired oracle；实现与资源
  均有效，sequential 与 flat 的 selected-cell hash 在 12/12 帧不同，且
  sequential 接受真实 depth$\ge2$ event。可是 sequential/flat mean CD 为
  $0.5494/0.1095$ m，CD 门通过 9/12 vs 12/12，target-stratum retention 为
  3/12 vs 4/12，exact-count/结构均为 9/12，first/later 完整门均为 0/12。
  因而 learned renewal 不授权。原拟议的 sparse ray-range partial transport
  又被首轮四位独立审计者判为命名和方法边界不成立：实际机制只是 Euclidean
  XYZ KNN 图上的稀疏矩形 assignment。修订路线已改名为 STDA-F0
  （sparse target-demand assignment），只作为内部容量 oracle，不作为算法创新。
  后续六轮 hostile audit 已对精确协议 SHA 给出 \code{FREEZE}；当前仅授权
  零训练实现，尚无 STDA 科学结果。
  ...
  因而不能主张“首次使用 Doppler 增密”或“首次多帧雷达增强”。最终可防守目标是
...
固定 range quota 审计 & \closed & 76/76 train 帧完成；2 帧的乐观 CD 下界越门，1 帧强制 20\% outlier & 硬 $8000/1700/300$ 配额退休；该 no-go 不否定 scorer 或 700k field \\
Q-Local-F0R & \closed & H200 GPU0 上 12/12 CD/outlier/结构通过；只有 2/12 target-stratum retention 通过 & pointwise global ranking 近距塌缩；scorer 未训练，终态 no-go \\
VRH-F0 renewal hazard & \closed & H200 GPU0 12 帧 paired run 完整；实现/资源与 activity 通过 & sequential mean CD $0.5494$ m vs flat $0.1095$ m；strata 3/12 vs 4/12，first/later 均 0/12，不授权训练 \\
STDA-F0 target-demand assignment & \pending & 六轮修订审计终局 \code{FREEZE}；all-76、双进程隔离、严格 5 cm、匹配控制与双副本事务已冻结 & 只授权零训练 oracle 实现；非 OT/ray-range 创新，无科学结果，不得训练 \\
R-B2 voxel-slot & \closed & GT-aided 结构容量通过；80k arm 的 recall 在 76/76 帧过门 & 3 帧 coverage 失败，当前 Cube-only activation family 关闭 \\
G4 时序数据 & \done & 45 序列、2,160 帧、6,660 文件、约 600.8 GiB；full-summary CRC 通过 & 只关闭数据完整性门；无合格 geometry parent，时序训练仍锁定 \\
...
大量雷达增强工作把任务写成稀疏点或 BEV 的去噪、补全与超分辨率，但相关领域
并非只有单帧方法：RadarHD 已用当前帧与 40 个历史帧重建当前稠密点云，
Radar-Mamba、RadarMP 和 DoppDrive 也分别覆盖历史特征增强、相邻 Cube 的点与
scene flow 联合估计以及 Doppler 驱动的历史点聚合。因此，本项目不能把
“首次使用多帧”或“首次用 Doppler 增强雷达点云”作为创新。更准确的定位是：
\begin{quote}
...
“使用历史帧”本身不能作为创新。RadarHD、Radar-Mamba、RadarMP、DoppDrive、
RMap、CMFlow 等已经覆盖时序重建、特征增强、scene flow 或历史聚合。
可防守的差异应同时满足以下四点：
\begin{itemize}
...
当前正式几何输出仍固定为 10,000 个互异点，并要求真实欧氏最小间距不低于
5 cm；禁止通过复制、padding、重复中心、后处理 jitter 或 best-of-$k$ 伪造点数。
早期协议还逐帧强制
\begin{equation}
  N^{\mathrm{old}}_{0-30}=8{,}000,\qquad
  N^{\mathrm{old}}_{30-60}=1{,}700,\qquad
  N^{\mathrm{old}}_{60-120}=300,
\end{equation}
但 76-frame 严格审计已证明该逐帧硬配额与冻结几何门不相容，因此它只作为
历史实验协议保留，不得进入新 exporter。新路线仍报告三个距离段的输出计数，
并对每个含 target 的距离段施加 completeness/recall 防塌缩门，而不是要求每一帧
产生固定距离质量。评估至少报告：
\begin{itemize}
...
固定配额审计绑定 source
\hash{dc63dfc404d647f31409177ccca6de80}{726eb8d7}，在 H200 主机上只读取
76 个 train target cache，11.2799 s 后原子落盘
\code{fixed\_range\_quota\_contract\_no\_go}。validation、test、Cube、CFAR、
model 与 candidate access 均为 false。其报告 SHA-256 为
\hash{d5d9eeaae99a97bb8d358ce2de7c6c48}{92e221e82b1e64eaf2897aa038cb627c}。
Q-Local-F0R 绑定 source
\hash{34579a245efecb0a10baeec3bcd1929a}{4744d502}，在物理 H200 GPU0 上用时
456.3370 s，峰值 allocated/reserved memory 为 1.827/2.179 GB，并原子落盘
\code{qlocal\_f0r\_capacity\_no\_go}。候选重建、旧 oracle 重放、scorer/loss
blob、parent checkpoint/metrics/manifest 与物理 GPU provenance 均通过；validation、
test、future access 和 model training 均为 false。报告 SHA-256 为
\hash{505644667ba3f73b58ee138608307e826}{32fe92bc3336f538c74767ab9c148ae}。
...
Q-Local-F0R exporter & 与 Q-Local-F0 bit-identical 的 700k candidates；全域稳定排序 + 5 cm greedy selection & exact 10k，不设逐帧 range quota；含 target 区间保留门 & H200 oracle 完成；2/12 完整通过，路线关闭 \\
VRH-F0 renewal hazard & $512\times214\times74$ frustum lattice；分离的 model marks/audit sidecar；每次 return 后 reset；显式 STOP & 同一 canonical event stream 的 sequential-frontier 与 flat-exposure 配对导出 & 已实现并完成 H200 paired oracle；activity 通过、utility no-go，训练未启动 \\
...
梯度更新，公式描述的是实现和已关闭的历史协议，不是已有训练结果；公式中的
$8000/1700/300$ 已由后续严格审计退休。
...
\end{equation}
每次更新确定性采样 16,000 个互异候选，按 $12800/2720/480$ 对三个距离段做
训练 minibatch 平衡；这不是推理输出配额。
...
冻结帧集合为
\begin{align*}
  \mathcal F_{\mathrm{fit}}={}&\{(1,232),(9,440),(21,402),(27,403),\\
  &\phantom{\{}(29,399),(35,403),(47,514),(58,404)\},\\
  \mathcal F_{\mathrm{unseen}}={}&\{(5,429),(26,403),(42,408),(50,405)\}.
\end{align*}
两组分别按固定环映射
...
邻域，故至少 20\% outlier 与网络无关。该发现触发并已完成独立的 76-frame
fixed-quota feasibility audit；下节给出其严格结论。

\subsection{固定逐帧距离配额：模型无关的不可行性证明}

对帧 $f$，记 target 的径向距离集合为 $T_f$，三个输出区间为 $I_k$，固定配额为
$q=(8000,1700,300)$。定义区间与最近 target range 的最小径向间隙
\begin{equation}
  \Delta_{f,k}=\inf_{r\in I_k}\min_{\rho\in T_f}|r-\rho|.
\end{equation}
对任何落入 $I_k$ 的预测点 $\mathbf x$ 与任意 target $\mathbf y$，反三角不等式给出
\begin{equation}
  \lVert\mathbf x-\mathbf y\rVert_2
  \ge \bigl|\lVert\mathbf x\rVert_2-\lVert\mathbf y\rVert_2\bigr|
  \ge \Delta_{f,k}.
\end{equation}
因此，即使完全忽略角度误差、候选场限制和 5 cm packing，并把 target-to-prediction
completeness 乐观地设为零，仍有
\begin{align}
  \mathrm{CD}_{\mathrm{LB}}(f)
  &=\frac{1}{10{,}000}\sum_k q_k\Delta_{f,k},\\
  \mathrm{Outlier}_{\mathrm{LB}}(f)
  &=\frac{1}{10{,}000}\sum_k q_k\,\mathbb I[\Delta_{f,k}>2\ \mathrm m].
\end{align}
这两个量只依赖 train target 的径向支撑；若下界越门，任何网络、候选表示或
排序方法都无法使该帧满足原 contract。

source \code{dc63dfc} 在 76/76 个 train 帧上完成审计，未读取 Cube、CFAR、
候选场、模型预测、validation 或 test。终态为
\code{fixed\_range\_quota\_contract\_no\_go}，耗时 11.2799 s。严格失败帧为：
\begin{table}[H]
\centering
\small
\caption{固定 $8000/1700/300$ 距离配额的严格不可行帧}
\begin{tabular}{lrrrrr}
\toprule
帧 & target 近/中/远 & 最大 range (m) & $\mathrm{CD}_{\mathrm{LB}}$ (m) & $\mathrm{Outlier}_{\mathrm{LB}}$ & 违反门 \\
\midrule
47:94 & 3286/2/0 & 32.6339 & 0.8210 & 3\% & CD \\
58:404 & 285/0/0 & 27.8774 & 1.3245 & 20\% & CD + outlier \\
\bottomrule
\end{tabular}
\end{table}
全部 76 帧中，强制 outlier 下界为 0、3\%、20\% 的帧数分别为 67、8、1。
由于冻结协议要求逐帧通过而不是只看均值，任一严格反例就足以否决整个硬配额
contract。该审计不推翻 Q-Local-F0 在原协议下的 terminal，也不证明 Q-Local
scorer 或 700k candidate field 失败；它只证明任何后继路线不得再继承逐帧硬
$8000/1700/300$ 配额。

\subsection{Q-Local-F0R：只修订 exporter 的最窄后继}

F0R 的问题是：移除已证明矛盾的 range quota 后，完全不变的 Fresh-WCE-20
700k candidate field 是否包含一个 exact-10k、真实 5 cm、逐帧过几何门的子集。
该路线保留 Q0/Q1、refined coordinates、residual、base confidence、Q-Local
scorer、8 fit + 4 unseen-train cohort 和稳定 candidate ID，只改一项机制：
\begin{equation}
  \operatorname{Export}_{\mathrm{F0R}}(s)=
  \operatorname{GreedyNMS}_{0.05\mathrm m}^{10{,}000}
  \bigl(\operatorname{sort}(-s,\operatorname{id})\bigr),
\end{equation}
即在 $[0,120)$ m 全域按分数降序、candidate ID 升序稳定排序，再执行真实
Euclidean 5 cm greedy selection，直到恰好 10,000 点。API 不接受 GT、target
range mass、替代 quota、padding、copy、jitter、best-of-$k$ 或 threshold sweep。

训练前先运行同一 12 帧的不可部署 GT-nearest capacity oracle。每帧仍须同时满足
$\mathrm{CD}\le0.8$ m、outlier $\le5\%$、exact 10k、unique 与 $d_{\min}\ge0.05$ m。
为防止取消硬配额后通过放弃真实中远距 target 获胜，对每个含 target 的距离段还要求
\begin{equation}
  \mathrm{Completeness}_{\mathrm{global}}
  \le \mathrm{Completeness}_{\mathrm{fixed}}+10^{-6},\qquad
  \mathrm{Recall@1m}_{\mathrm{global}}
  \ge \mathrm{Recall@1m}_{\mathrm{fixed}}-10^{-6}.
\end{equation}
source \code{34579a2} 在物理 H200 GPU0 完成正式运行，原子终态为
\code{qlocal\_f0r\_capacity\_no\_go}。12/12 帧均满足 exact 10k、unique、真
5 cm、CD $\le0.8$ m 和 outlier $\le5\%$；所有帧的 2 m outlier 实际均为 0。
但只有 47:514 与 58:404 两帧通过全部 target-stratum retention，完整门仅为
2/12。逐帧结果如下：

\begin{table}[H]
\centering
\scriptsize
\caption{Q-Local-F0R 全域 GT-nearest capacity oracle；近/中/远对应
$[0,30)/[30,60)/[60,120)$ m}
\begin{tabular}{llrrrl}
\toprule
帧 & group & CD (m) & outlier & selected 近/中/远 & strata retention \\
\midrule
1:232 & fit & 0.7941 & 0 & 9535/441/24 & fail \\
9:440 & fit & 0.3089 & 0 & 9777/219/4 & fail \\
21:402 & fit & 0.3936 & 0 & 9542/436/22 & fail \\
27:403 & fit & 0.2814 & 0 & 9967/32/1 & fail \\
29:399 & fit & 0.2814 & 0 & 9798/179/23 & fail \\
35:403 & fit & 0.3763 & 0 & 8506/1493/1 & fail \\
47:514 & fit & 0.4971 & 0 & 9904/96/0 & pass \\
58:404 & fit & 0.6548 & 0 & 10000/0/0 & pass \\
5:429 & unseen train & 0.4203 & 0 & 9858/140/2 & fail \\
26:403 & unseen train & 0.3878 & 0 & 9751/247/2 & fail \\
42:408 & unseen train & 0.4385 & 0 & 9693/276/31 & fail \\
50:405 & unseen train & 0.3996 & 0 & 9630/274/96 & fail \\
\bottomrule
\end{tabular}
\end{table}

这同时给出两个边界。第一，取消硬配额确实修复了旧失败帧：47:514 与 58:404
分别达到 CD $0.4971/0.6548$ m 且 outlier 为 0。第二，全域 pointwise 最近距离
排序不是可部署解：它在 10 帧把 85.06\%--99.67\% 的预算投向 30 m 内；例如
1:232 的 far completeness 从固定 oracle 的 0.8063 m 恶化到 3.5947 m，35:403
只选择一个 far point，far completeness 达 18.1071 m。失败是集合级 range
allocation 与 ordered-return representation 的不足，而不是 aggregate candidate
support 不足。

因此 Q-Local scorer 不启动 500-update 训练；也不能用 aggregate Chamfer 通过、
删除帧或放宽 strata 门重开。F0R 的有效科学 no-go 按冻结协议授权
variable multi-return renewal-hazard capacity route。固定 $K=4/6$ measured-peak、
旧硬配额与 pointwise global ranking 均不得重开。

\subsection{VRH-F0：可变多回波续接 hazard 配对容量协议}

VRH-F0 候选协议在任何实现前固定唯一分辨率：把原生
$(R,A,E)=(256,107,37)$ 的每个自然 Voronoi cell 二等分，形成
\begin{equation}
  (R_2,A_2,E_2)=(512,214,74),\qquad
  A_2E_2=15{,}836\ \text{subrays},
\end{equation}
共 8,108,032 个 target-independent ray--range cells。range 上界保持原生末端
半步的 $118.2685546875$ m，不人为扩展到 120 m。每个 cell 最多携带一个
bounded $(\Delta r,\Delta a,\Delta e)$ event；cell ID、axes、bounds 与完整 support
在 target loader 之前构造和哈希。

GT 只能进入非部署函数 \code{fit\_gt\_oracle\_marks}。它产生两个物理隔离的
artifact：decoder 可见的 \code{ModelMarks} 仅含 finite hazard、bounded offsets、
opaque integer priority 与 confidence；\code{FitSidecar} 才保存 nearest-target ID、
真实 fitted distance 与 exact-coordinate coincidence。decoder/exporter 的签名、
配置、import 与 opened files 均不得包含 target rows、target IDs、sidecar 或
预选 mask。

对第 $q$ 条 subray，第 $m$ 次续接从 cursor $c_m$ 开始，下一 event 的语义为
\begin{equation}
 p(B_m=b\mid H_m)=h_{q,b}(H_m)
 \prod_{j=c_m}^{b-1}\bigl(1-h_{q,j}(H_m)\bigr),
 \qquad
 p(\mathrm{STOP}\mid H_m)=
 \prod_{j=c_m}^{R_2-1}\bigl(1-h_{q,j}(H_m)\bigr).
\end{equation}
命中后 cursor 前移、survival interval 重置，并施加固定 5 cm refractory rule；
返回数只受 512 个 range cells 的有限 support 约束，不存在固定 $K$ 或 range quota。
两种 exporter 运行前，decoder 先把每条 ray 一次性展开到 STOP，并冻结唯一
canonical event-stream hash。

决策臂每条 ray 只暴露当前 next event；被消费后才暴露下一 event。control 臂则把
完全相同的 canonical stream 一次性平铺到同一 heap。两臂共享 marks、events、
scores 与 5 cm greedy check，只改变 frontier exposure。12 帧均逐帧评估 CD
$\le0.8$ m、outlier $\le5\%$、exact 10k、真 5 cm、F0R range-stratum retention，
并用同 ray、range-monotone、event 不可复用的匹配检查 first/later groups：
completeness $\le1.0$ m、recall@1m $\ge0.80$。

只有 sequential 决策臂在 12/12 帧通过全部门、flat control 至少失败一个对应门，
且两臂 selected IDs 确有差异并接受真实 depth$\ge2$ event，才能得到
\code{vrh\_f0\_capacity\_passed}。若两臂都通过，只能记录 lattice/mark-field
capacity，不能授权 renewal。协议不读取 Cube/CFAR；即便 capacity pass，也只授权
另行冻结的 target-free one-frame learnability/condition-use gate，不解锁 Doppler、
cycle、temporal、validation 或 test。

\paragraph{正式结果。}
source \code{380f3ea} 在物理 H200 GPU0 完成全部 12 帧、两个 arm 的正式运行。
support、model marks、audit sidecar、canonical stream、export replay、独立
KD-tree spacing、API 边界与资源门全部有效；总 wall time 为 325.07 s，峰值 host
RSS 为 1.78 GiB，峰值 CUDA reserved 为 110 MiB。因此终态是科学 no-go，
不是实现或资源无效。

\begin{table}[H]
\centering
\small
\caption{VRH-F0 同一 canonical stream 上的 paired capacity 结果}
\begin{tabular}{lrrrrrr}
\toprule
Arm & mean CD & median CD & CD 门 & 结构门 & strata 门 & first/later 门 \\
\midrule
Sequential frontier & 0.5494 m & 0.3692 m & 9/12 & 9/12 & 3/12 & 0/12 \\
Flat exposure & 0.1095 m & 0.1055 m & 12/12 & 9/12 & 4/12 & 0/12 \\
\bottomrule
\end{tabular}
\end{table}

两臂的 outlier 门均为 12/12。Sequential 与 flat 的 accepted-cell hash 在
12/12 帧不同，且 sequential 在全部帧接受 depth$\ge2$ event，故 renewal
activity 得到认证；但 utility 未认证。Sequential 不仅没有形成任何
decision-pass/control-fail 对应门，平均 CD 还约为 flat 的 5.02 倍。47:514、
58:404 和 50:405 在真实 5 cm 检查后分别只能导出 4,359、1,602 和 9,993 点；
全部 66 个非空 first/later range-class 中，两臂都只有 1 个通过。

正式终态为 \code{vrh\_f0\_capacity\_no\_go}。这关闭冻结的 GT-fitted
$2R\times2A\times2E$ renewal/frontier 配方，不证明所有 ray-native process
都不可能；但该结果不能授权 learned renewal。后继草案经首轮四位独立审计者后已从
``sparse ray-range partial transport'' 修正为 STDA-F0：在 target-independent
5 cm parity support 上求 graph-constrained sparse bipartite assignment。RAE 不进入
图或代价，assignment/partial-transport 原语已有先例，故它只检验集合分配容量，
不是创新点。协议随后经过六轮逐字 hostile audit，最终对 SHA-256
\code{a1bacd61\ldots edf84} 获得 \code{FREEZE}。这使 all-76 零训练 oracle
实现具备资格，但在正式 H200 事务完成前仍没有容量结论；learned 网络继续禁止。
...
进一步代码与日志审计表明，exposure bias 不是唯一病因：桥式生成在第一个
generated step 已显著损伤几何，说明 full-set refiner 正在重写本来较强的
\code{copy\_dopp} scaffold；scheduled sampling 只能改善后段稳定，不能消除
该一步税。旧 evaluator 还在各 arm 间使用不同随机初始/GT 子采样，因此小幅差异
必须先做 paired replay 才能进入论文。若单帧 geometry parent 将来通过，时序实验
顺序固定为：修复 paired evaluator；bitwise 保留 XYZ 的 copy-anchored Doppler
refresh；零门控、有界残差和 no-harm hinge 的 trust-region RaLD refiner；
dynamic/confidence selective renewal；最后才考虑 free-running distillation、
scene-flow/occupancy state 和 latent recurrence。不能再次把 scheduled sampling
当作一步几何退化的替代解释。
...
\midrule
\endhead
\href{https://ojs.aaai.org/index.php/AAAI/article/view/38946}{RaLD} &
Radar spectrum 条件的 occupancy field 与 latent generation &
continuous frustum queries、mixed latents、逐层 radar condition &
作为 VRH 后续 target-free conditioner；不照搬 thresholded intensity-only export \\

\href{https://arxiv.org/abs/2405.05131}{DenserRadar} &
Full-Doppler 3D U-Net 输出稠密 occupancy &
固定 $2R\times2A\times2E$ frustum resolution 与 deep supervision &
只冻结 VRH lattice 分辨率；不照搬独立 occupancy threshold \\
...
\href{https://arxiv.org/abs/2206.09273}{RadarHD} &
当前雷达图像与 40 个历史帧（2 s）联合重建当前稠密点云 &
历史持久性与当前观测融合 &
直接否定“雷达增强此前只有单帧”的表述；作为 temporal reconstruction control \\
...
\href{https://arxiv.org/abs/2512.14235}{4D-RaDiff} /
\href{https://arxiv.org/abs/2512.17897}{RadarGen} &
由 box/LiDAR 或多视角图像条件生成带雷达属性的点云 &
属性生成、条件生成与雷达仿真评估协议 &
否定“首次生成 Doppler 雷达点”的表述；与 Cube 测量闭环任务作明确区分 \\

\href{https://arxiv.org/abs/2608.04130}{Radar4D-VLM} &
十帧 4D radar sweep 的场景与运动推理 &
时序 radar-only 状态编码与径向速度 token &
说明时序雷达状态推理正在快速发展；不等同于 dense point-state forecasting \\

\href{https://openaccess.thecvf.com/content/ICCV2023/papers/Huang_Neural_LiDAR_Fields_for_Novel_View_Synthesis_ICCV_2023_paper.pdf}{Neural LiDAR Fields} &
beam divergence、ray drop 与第二回波截断 rendering &
命中后清空前段 density 并 reset transmittance &
迁移 reset 原理；不继承固定双回波与逐场景 LiDAR 优化 \\

\href{https://arxiv.org/abs/2411.01725}{PLiNK} &
沿 ray 的概率积分/CDF，可采样 first、nth、strongest return &
range hazard 与多峰监督 &
增加 renewal/STOP 后用于 VRH；单一多峰 CDF 不等于 event sequence \\

\href{https://papers.neurips.cc/paper_files/paper/2017/hash/6463c88460bd63bbe256e495c63aa40b-Abstract.html}{Neural Hawkes} &
由历史事件状态控制下一事件 intensity &
ordered event-conditioned state &
只迁移 next-event 语义；不主张 radar return 服从 Hawkes 自激 \\

\href{https://arxiv.org/abs/2403.10094}{RangeLDM} &
range-native compact latent generation &
sensor-aligned range 表征 &
相关表示参考，不替代 VRH 的 variable multi-return capacity gate \\
\bottomrule
\end{longtable}

本轮一级文献检索尚未发现一个完全匹配的任务：以历史生成状态和当前完整
RAED Cube 为条件，自回归地产生下一时刻稠密
$\{\xyz,q(v_r),\text{confidence}\}$ 状态，并同时约束 Cube 重投影和
位移--径向速度闭环。但这只是截至本报告日期的限定检索结果，不是文献不存在的
证明，也不能写成 ``first''。投稿前必须沿 RaLD、RadarMP、DoppDrive、SDDiff、
RaUF、4D-RaDiff 和 Radar4D-VLM 的引用图再次复核。
...
\subsection{P1（已完成）：Q-Local-F0 source-bound tiny gate}
...
source \code{bf1a166} 的静态与 H200 回归通过。真实 12-frame preflight 的原子
终态为
\begin{center}
  \code{qlocal\_f0\_capacity\_no\_go}。
\end{center}
其中 10 帧通过，47:514 和 58:404
分别以 $1.4250$ m/$7.0\%$ 与 $2.2598$ m/$20.0\%$ 的 CD/outlier 失败。
scorer 训练、optimizer state 和 learned checkpoint 均未创建，Q-Local 路线关闭。

原预注册决策要求直接转向 variable multi-return ray-hazard，但 58:404 同时证明旧
per-frame range quotas 可能使任何表示在 Chamfer/outlier 门下不可行。随后完成的
76-frame fixed-quota feasibility audit 给出两个严格反例，故硬配额退休；最窄
后继 F0R 只取消该配额并保留其余机制。

F0R 已在 source \code{34579a2} 上正式完成：全部 12 帧的 CD/outlier/structure
通过，但 target-stratum retention 只有 2/12。该结果是有效科学 no-go，故
Q-Local scorer 仍未训练，并按协议切换到 variable multi-return renewal-hazard
容量路线。VRH-F0 随后也完成有效 H200 paired run：renewal activity 通过，但
sequential utility、strata、first/later 与三帧 exact-count 未形成完整通过，故
learned renewal 同样未启动。任何后继都不能删除失败帧、改成 aggregate-only
gate、事后放宽几何门，或恢复固定 $K$、硬 range quota 与当前 VRH 配方。

\subsection{G4 时序数据 full-summary CRC}

独立数据完整性任务在 H200 host \code{WHUServer-H200}、用户 \code{wangning}
完成，verifier source label 为 \code{083f4c4}。最终
\code{verification.json} 给出：2,160/2,160 Cube、LiDAR 与 label 帧齐全，
6,660 个文件通过校验，45/45 sequence member sets 与 manifest 完全一致，且
\code{invalid\_files=[]}、\code{errors=[]}、pending/active sequence 均为空。
输出 SHA-256 为
\hash{0ed646c841ccfd88863cae6333c7b065}{016425c96d96eb97c53b941f01470fda}，
完整 time/log SHA-256 为
\hash{9c6bcd5281a0007b424baf2b9799de8a}{4b308c53febca4661d06df3b810c0df4}。

审计 wall time 为 1:56:21，最大常驻内存 159,940 KiB；manifest 估算 Cube 加
LiDAR payload 为 600.773 GiB。该任务是 I/O/CPU-bound 数据门，不是 GPU 模型实验。
它只证明 byte-level manifest agreement、文件计数和完整成员集；不证明 label
正确性、标定质量或任何时序指标。validation 文件仅做 checksum，没有用于拟合、
指标选择或路线选择；test 仍未访问。由于尚无合格单帧 geometry parent，G4 训练
保持锁定。
...
P2 & fixed range-quota audit & 已完成，76/76 train 帧，11.2799 s & 2 帧严格不可行；旧硬配额终止且不得复用 \\
F0R & global-export candidate oracle & 已完成，456.34 s，H200 GPU0 & 12/12 几何/结构通过，2/12 strata retention；科学 no-go \\
F1 & VRH-F0 renewal-hazard oracle & 已完成，325.07 s，H200 GPU0 & activity 通过；sequential utility no-go，不授权 learned renewal \\
F2 & STDA-F0 sparse target-demand assignment & 协议已冻结，零训练实现待完成 & all-76 容量、matched controls、资源与双设备证据门；原语不作为创新 \\
P3 & Doppler + Cube-cycle & 锁定 & 无合格 geometry parent \\
P4 & RadarMP + DoppDrive 时序 & 锁定 & G4 full-summary CRC 已通过；仍无合格单帧 family \\
\bottomrule
\end{tabularx}
\end{table}
...
又在训练前的两级容量门停止。固定 range quota 审计已经证明旧输出契约不可复用，
F0R 又证明无配额 pointwise global ranking 会丢失中远距 strata。因此目前没有
活动的 learned fixed-10k 几何训练。VRH-F0 已进一步证明：在同一 fitted mark
stream 上，sequential renewal 虽真实改变选择并接受后续回波，却没有带来几何、
strata 或 first/later 效用，因而该 learned route 也未获授权。当前只剩已经
外部 SHA 绑定冻结、尚待实现和执行的 STDA-F0 零训练容量门。它明确是
graph-constrained target-demand assignment，不是 ray-range transport、hard
rounding 或新的 OT 算法；协议冻结本身不是实验结果。
当前仍应停止扩展完整 downstream 叙事，直到新的 geometry parent 通过；不能用
diffusion 或 temporal complexity 掩盖上游表示与输出约束的矛盾。
...
截至 2026-08-08 本次归档截点，项目已经完成从“宽泛的 Cube-to-dense 设想”
...
76-frame train-only quota feasibility audit 已完成并严格否决旧逐帧硬配额。
随后完成的 F0R 又提供更细的终止证据：同场 700k field 在无配额 GT-nearest
global export 下 12/12 通过 CD、outlier、count 与 spacing，但只有 2/12 保留
全部 target-bearing strata。取消配额解决了旧失败帧，却不能解决近距预算塌缩。
VRH-F0 现已完成这一直接检验：sequential renewal activity 确实存在，但
mean CD 从 flat 的 $0.1095$ m 恶化到 $0.5494$ m，strata retention 只有 3/12，
first/later 完整门为 0/12，且三帧无法在真实 5 cm 下达到 exact 10k。
因此当前最优先工作不再是增加网络深度、训练 Q-Local 或训练 renewal，而是按
已经冻结的 STDA-F0 协议实现 oracle，再以全部 76 个 train 帧检验 graph-constrained
target-demand assignment 能否在不恢复硬配额的前提下同时通过绝对几何、range
strata 与 representation-neutral first/later 门。
...
\small
\begin{longtable}{>{\raggedright\arraybackslash\bfseries}p{4.0cm} >{\raggedright\arraybackslash}p{10.4cm}}
\toprule
项目 & 冻结信息 \\
\midrule
\endfirsthead
...
固定配额审计提交 & \hash{dc63dfc404d647f31409177ccca6de80}{726eb8d7} \\
Q-Local-F0R 提交 & \hash{34579a245efecb0a10baeec3bcd1929a}{4744d502} \\
...
时序数据 & 45/45 序列、2,160 帧、6,660 文件；约 600.8 GiB；full-summary CRC 已通过并归档 \\
当前输出契约 & exact 10,000 points + 真实 5 cm；旧逐帧 $8000/1700/300$ 配额已由严格审计退休 \\
最小点距 & 5 cm \\
测试访问 & false \\
...
固定配额审计 terminal & \code{fixed\_range\_quota\_contract\_no\_go}；76/76 train 帧，validation/test access = false \\
固定配额报告 SHA &
\hash{d5d9eeaae99a97bb8d358ce2de7c6c48}{92e221e82b1e64eaf2897aa038cb627c} \\
Q-Local-F0R 状态 & \code{qlocal\_f0r\_capacity\_no\_go}；12/12 CD/outlier/结构通过，2/12 strata retention，训练未启动 \\
Q-Local-F0R 报告 SHA &
\hash{505644667ba3f73b58ee138608307e826}{32fe92bc3336f538c74767ab9c148ae} \\
Q-Local-F0R GPU & physical 0；UUID \code{GPU-74b9b73f-c405-55bc-bf76-f8aa85d1e7fe}；PCI \code{00000000:01:00.0} \\
VRH-F0 执行提交 & \hash{380f3eaef336bee5a019eff5121eeacceb6e2066}{} \\
VRH-F0 终态 & \code{vrh\_f0\_capacity\_no\_go}；activity pass，utility fail；sequential/flat mean CD $0.5494/0.1095$ m \\
VRH-F0 full report SHA &
\hash{5393576d905bc8344384fcf4c9e19433}{4c26ee1b3ed58e3e61460142181c5054} \\
VRH-F0 compact report SHA &
\hash{944b3e338823498ae61c545b067bf92db}{d0d3600f1aef6aa099eb7dc498c1eed} \\
VRH-F0 GPU & physical 0；UUID \code{GPU-74b9b73f-c405-55bc-bf76-f8aa85d1e7fe}；PCI \code{00000000:01:00.0} \\
STDA-F0 协议状态 & 外部 SHA 绑定 \code{FREEZE}；仅授权零训练实现，尚无科学运行结果 \\
STDA-F0 协议 SHA &
\hash{a1bacd619ab462b92e8b0fbfccf142e9}{999ab18130996762f6964a3ccd2edf84} \\
STDA-F0 round-6 audit SHA &
\hash{5b27abd63a114c1f107dd22b65590b39}{a5425c60dc72809d3bc6ce8027ade7d5} \\
STDA-F0 freeze-record SHA &
\hash{af0134038cb501b03fe1dd25937c46f9}{bdfacbca08fa5c6068afbc3dcfe42c7d} \\
G4 CRC terminal & \code{g4\_temporal\_crc\_passed}；2,160 帧、6,660 文件、45/45 member sets、0 invalid/error \\
G4 verification SHA &
\hash{0ed646c841ccfd88863cae6333c7b065}{016425c96d96eb97c53b941f01470fda} \\
RaLD 审计 commit &
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.1em,topsep=0.15em]
  ...
  \item \path{artifacts/idea/literature_positioning_2026-08-07.md}
  \item \path{docs/qlocal_distributional_risk_tiny_protocol.md}
  \item \path{docs/fixed_range_quota_feasibility_audit.md}
  \item \path{docs/qlocal_f0r_global_export_capacity_protocol.md}
  \item \path{artifacts/g1/qlocal_f0_preflight_bf1a166/preflight.json}
  \item \path{artifacts/g1/fixed_range_quota_audit_dc63dfc/feasibility.json}
  \item \path{artifacts/g1/fixed_range_quota_audit_dc63dfc/decision.md}
  ...
  \item \path{docs/qlocal_f0r_global_export_capacity_protocol.md}
  \item \path{docs/vrh_f0_variable_return_capacity_protocol.md}
  \item \path{docs/stda_f0_sparse_target_demand_assignment_protocol.md}
  \item \path{artifacts/idea/stda_f0_prefreeze_audit_round1.md}
  \item \path{artifacts/idea/stda_f0_prefreeze_audit_round6.md}
  \item \path{artifacts/idea/stda_f0_freeze_record.json}
  \item \path{artifacts/idea/stda_f0_prefreeze_solver_benchmark.md}
  \item \path{artifacts/g1/qlocal_f0r_global_export_34579a2/decision.md}
  \item \path{artifacts/g1/vrh_f0_capacity_380f3ea/decision.md}
  \item \path{artifacts/g1/vrh_f0_capacity_380f3ea/preflight_compact.json}
  \item \path{artifacts/scout/2026-08-07_rald_temporal_literature.md}
  \item \path{artifacts/idea/long_horizon_candidate_frontier.md}
  \item \path{artifacts/g4/g4_temporal_crc_083f4c4/verification.json}
  \item \path{artifacts/g4/g4_temporal_crc_083f4c4/decision.md}
...
\begin{itemize}[itemsep=0.15em,topsep=0.2em]
  ...
  \item 本报告没有把 Q-Local capacity no-go 泛化成所有 continuous quality scorer 失败；scorer 从未训练。
  \item 本报告把 F0R 写成 H200 capacity no-go，而不是 learned result；12/12 aggregate geometry 通过与 2/12 strata retention 通过同时保留。
  \item 本报告没有把 F0R 的 pointwise global-ranking no-go 泛化成所有 set allocation 或 ray-native representation 失败。
  \item 本报告把 VRH-F0 写成有效 H200 capacity no-go，而不是 learned result；activity pass 与 utility fail 同时保留。
  \item 本报告没有把冻结的 VRH-F0 配方失败泛化成所有 renewal、ray-native process 或 partial transport 失败。
  \item 本报告没有把 STDA-F0 写成 ray-range transport、新 OT 算法、hard rounding、唯一最优解或可部署模型；协议已冻结，但 GT-conditioned capacity oracle 尚未实现或执行。
  \item 本报告没有把 G4 CRC 通过写成时序模型通过；它只验证数据文件与 manifest 完整性。
  ...
  \item 本报告没有声称首次多帧雷达增强、首次生成 Doppler 雷达点或首次联合 spatial--Doppler 建模；未检索到完全匹配任务只作为限定检索结果。
