...
固定 $8000/1700/300$ 距离配额可能与 5\% outlier 门矛盾；76-frame train-only
...
\begin{enumerate}
  ...
  固定逐帧 range quota 可能给出与 5\% 门无关模型的矛盾下界。当前先对全部
  76 个 train 帧做 reverse-triangle-inequality 审计；若任一帧的强制 outlier
  下界超过 5\%，下一 ray-hazard 路线不得继承旧配额。
...
固定 range quota 审计 & \running & 已冻结严格径向 outlier 下界与 76-frame train-only 协议 & 正在执行；决定 ray-hazard 是否必须采用自适应 range mass \\
...
feasibility audit；在其完成前，不实现沿用相同配额的 ray-hazard。
...
per-frame range quotas 可能使任何表示在 5\% outlier 门下不可行。因此先执行
独立 76-frame fixed-quota feasibility audit。若审计 no-go，ray-hazard 必须采用
另行冻结的 target-free Cube-predicted adaptive range mass 或 variable cardinality；
不能简单删除失败帧或事后放宽 Q-Local 门。稀疏 ray-range partial transport 排在
第三位，仍须先满足 rounding gap、2 s/frame 与 60 GiB 资源门。
...
P2 & fixed range-quota audit & 76 train 帧，零训练 & 运行中；检查 5\% outlier 的严格径向下界 \\
F1 & variable ray-hazard oracle & 后备，零训练 & 等待 P2 决定固定配额或自适应 range-mass contract \\
