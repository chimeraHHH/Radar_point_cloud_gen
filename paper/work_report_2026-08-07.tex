...
\code{qlocal\_f0r\_capacity\_no\_go}。Q-Local scorer 仍未训练；VRH-F0
variable multi-return renewal-hazard paired capacity protocol 已通过独立终审并冻结。
...
\begin{enumerate}
  ...
  scorer 训练继续禁止；VRH-F0 variable multi-return renewal-hazard 配对容量
  协议已冻结，下一步才允许实现和执行。
...
VRH-F0 renewal hazard & \pending & 协议已通过独立终审并冻结：单一 $512\times214\times74$ lattice、model-mark/GT-sidecar 隔离、canonical reset/STOP streams & 只有 sequential arm 全过且同流 flat arm 至少失败一个对应门，才授权 renewal；尚无 H200 结果 \\
...
VRH-F0 renewal hazard & $512\times214\times74$ frustum lattice；分离的 model marks/audit sidecar；每次 return 后 reset；显式 STOP & 同一 canonical event stream 的 sequential-frontier 与 flat-exposure 配对导出 & 协议已通过独立终审并冻结；实现与 H200 结果待完成 \\
...
容量路线。任何后继都不能删除失败帧、改成 aggregate-only gate、事后放宽几何门，
或恢复固定 $K$ 与硬 range quota。
...
F1 & VRH-F0 renewal-hazard oracle & 当前 live gate，零训练 & paired protocol 已通过独立终审并冻结；下一步实现并在同一 12-frame train-only cohort 正式运行 \\
...
活动的 learned fixed-10k 几何训练；当前活动路线是协议已通过独立终审并冻结、尚未实现的
VRH-F0 variable multi-return renewal-hazard 零训练 paired capacity oracle。
...
因此当前最优先工作不再是增加网络深度或训练 Q-Local，而是完成 VRH-F0 paired
protocol 终审，随后实现并执行 variable multi-return renewal-hazard capacity oracle，
用同一 canonical stream 的 sequential-vs-flat 对照直接检验 renewal 是否真正改善
集合级多距离分配能力。
...
VRH-F0 协议状态 & independent audit = FREEZE；model-mark/GT-sidecar 隔离，sequential-vs-flat utility gate；H200 结果尚不存在 \\
...
\begin{itemize}[itemsep=0.15em,topsep=0.2em]
  ...
  \item 本报告没有把 VRH-F0 的候选协议、文献动机或 8,108,032-cell support 写成 H200 capacity 结果。
