...
\code{qlocal\_f0r\_capacity\_no\_go}。Q-Local scorer 仍未训练；当前转入另行
冻结的 variable multi-return renewal-hazard capacity oracle。逐点 Doppler、
...
\begin{enumerate}
  ...
  scorer 训练继续禁止，下一步转入 variable multi-return renewal-hazard 表示。
...
Variable renewal hazard & \pending & F0R 科学 no-go 已授权零训练表示容量测试 & 协议须在实现前冻结；固定 $K=4/6$、硬 range quota 与 pointwise global ranking 禁止复用 \\
...
Variable renewal hazard & azimuth/elevation ray 上的 survival hazard；每次 return 后 renewal/reset；返回数可变 & ordered multi-return field，后接 exact-10k/5 cm 全局导出 & 当前 live capacity hypothesis；协议待冻结，无模型结果 \\
...
F1 & variable renewal-hazard oracle & 当前 live gate，零训练 & 先冻结 variable-return 表示与 anti-collapse 容量协议，再实现和运行 \\
...
活动的 learned fixed-10k 几何训练；当前活动路线是另行冻结的 variable
multi-return renewal-hazard 零训练 capacity oracle。
...
因此当前最优先工作不再是增加网络深度或训练 Q-Local，而是冻结并执行 variable
multi-return renewal-hazard capacity oracle，用 ordered return renewal 直接检验
集合级多距离分配能力。
...
当前几何容量路线 & variable multi-return renewal hazard；零训练协议待冻结 \\
RaLD 审计 commit &
