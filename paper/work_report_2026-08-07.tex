...
重放没有通过预注册的原模型端点等价门，因此八帧 quality training 在启动前跳过。
原 R-A1 checkpoint 文件已在存储清理中删除；新 checkpoint 只能称为
\textbf{同源同协议重放}，不得称为原 checkpoint 的同一复现。
当前没有获授权的 fixed-10k 几何训练分支，逐点 Doppler、Cube-cycle、
时序生成和测试集评估仍未解锁。
\end{minipage}}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{当前 fixed-10k 几何 family 已触发止损。}
  R-B2 与 Q1-R 两条末端路线均按预注册门停止，不能继续叠加 diffusion 或 temporal
  模块。下一步若继续科研，必须建立机制上不同、重新冻结且保留未触碰 test 的
  新命名几何分支；当前 24 帧只能作为 development evidence，不能称为独立验证。
\end{enumerate}
...
连续质量评分优于 binary occupancy & 未评估 & quality head 已实现，但 replay-parent 证书 no-go，训练未启动 \\
...
因此，Q1-R 的 continuous quality target 是由当前故障与 quality-aware
...
P1 & Q1-R 八帧 tiny & 未启动 & 父证书 no-go，非模型训练失败 \\
P2 & Q2 RaUF-lite & 未启动 & Q1-R 条件未成立 \\
P3 & Doppler + Cube-cycle & 锁定 & 无合格 geometry parent \\
P4 & RadarMP + DoppDrive 时序 & 锁定 & 无合格单帧 family \\
\bottomrule
\end{tabularx}
\end{table}
...
\end{quote}
R-B2 当前 activation family 已失败，Q1-R 又在 replay-parent 授权门停止，
因此现有工作计划内已经没有活动的 fixed-10k 几何训练路线。当前应停止扩展
完整方法叙事，把阶段成果收敛为数据/评估协议、系统性负结果与候选选择诊断；
若继续投稿主线，需要重新定义机制上不同的几何任务或输出契约，而不是继续堆叠
diffusion 与时序模块。
...
“只是 RaLD 加 Doppler” &
RaLD 结构大量进入候选路线 &
明确 RaLD 的 variable threshold 边界；用 Q1-R、逐点分布和 Cube closure
建立不同任务与机制 \\
