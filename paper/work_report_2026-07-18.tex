...
\midrule
\endhead
G0 & 数据、坐标、同步和物理轴是否可靠 & 100 帧 CUDA 审计全部通过，CRC、CFAR round-trip、时间参考、可见性和静态 Doppler 约定一致 & \statusrun，首轮完成但 9/11，修复重跑 \\
...
首轮正式 G0 已在 H200 GPU0 上完成 100/100 帧，但 gate failed；失败决策已归档，nonempty 逻辑已修正并补充回归测试。下游 G1 preflight、G1 formal、static Doppler audit、G2/G3 均不得越过失败 aggregate。修复版 G0 将使用 no-deskew target cache 和新 source commit 独立重跑。
...
报告前代码 HEAD & \code{14e16d5ddf82685f1c5725551eb3aba84861376c} \\
