...
> **2026-08-07 Q-Local-F0 与输出契约终局修订：**新命名 source `bf1a166` 在物理 H200 GPU2 完成 12-frame train-only preflight。700k candidate/Q0/Q1/base-confidence、exact-10k/5 cm、六组 scorer 梯度、same-coordinate wrong-Cube、raw-input/source/GPU/atomic provenance 均通过，但 GT-nearest capacity oracle 只通过 10/12 帧；47:514 与 58:404 的 CD/outlier 分别为 `1.4250 m/7.0%` 与 `2.2598 m/20.0%`，故终态 `qlocal_f0_capacity_no_go`，380,818 参数 scorer 从未训练。独立 source `dc63dfc` 随后只读取 76 个 train target cache，用反三角不等式给出模型无关下界：47:94 与 58:404 的乐观 CD 下界为 `0.8210 m/1.3245 m`，后者还被固定 `8000/1700/300` 配额强制产生至少 20% outlier。终态 `fixed_range_quota_contract_no_go`，因此硬逐帧 range quota 永久退休，但 exact 10k 与真实 5 cm 保留。当前最窄后继为 [`Q-Local-F0R`](qlocal_f0r_global_export_capacity_protocol.md)：保持 Fresh-WCE 700k field 与 Q-Local scorer/loss 不变，只改全域 exporter；先做同一 12 帧零训练 capacity oracle，并保护所有 target-bearing range strata。只有 F0R 科学 no-go 才授权 variable multi-return renewal-hazard，implementation-invalid 不得路由。
...
- [ ] 完成 Q-Local-F0R 全域 exact-10k/5 cm capacity oracle，并按科学 pass/no-go 路由到 500-update scorer 或 variable renewal-hazard。
...
> 当前已无获授权的 fixed-10k 几何训练分支：Q1-R 因 replay-parent 证书 no-go 在训练前停止，Q-Local-F0 因旧 exporter capacity no-go 停止。当前唯一 live gate 是新命名且独立冻结的 Q-Local-F0R 零训练 oracle；Q2 和全部下游物理/时序路线保持锁定，test 未触碰。不得把当前 replay 追溯认定为原 R-A1 checkpoint，也不得恢复已证明矛盾的逐帧 hard range quota。
