...
from models.temporal_prior import ego_pose_warp, gated_doppler_warp
...
def rald_history_aggregate(
    current: RaLDPointPrediction,
    warped_history_newest_first: list[RaLDPointPrediction],
    point_count: int,
    spatial_shape: tuple[int, int, int] = (256, 107, 37),
    recency_time_constant_frames: float = 4.0,
) -> tuple[RaLDPointPrediction, AggregationDiagnostics]:
    """Convention-free confidence/recency aggregation for the G4R control."""

    if point_count < 1:
        raise ValueError("G4R history point count must be positive")
    if recency_time_constant_frames <= 0.0:
        raise ValueError("G4R recency time constant must be positive")
    aged_states = [(0, current)]
    for age, history in enumerate(warped_history_newest_first, start=1):
        valid = in_spatial_grid(history.coordinates_rae, spatial_shape)
        if valid.any():
    ...
    if xyz.shape[0] < point_count:
        raise ValueError(
            f"G4R aggregation has {xyz.shape[0]} candidates for {point_count} outputs"
        )
    ages = torch.cat(
        [
            torch.full(
    ...
    recency = torch.exp(-ages / recency_time_constant_frames)
    candidate_index = torch.arange(xyz.shape[0], device=xyz.device)
    score = confidence.clamp_min(0.0).double() * recency.double()
    score = score + (xyz.shape[0] - candidate_index).double() * 1e-12
    quantized = torch.floor(coordinates).long()
    quantized = torch.stack(
        [
            quantized[:, axis].clamp(0, spatial_shape[axis] - 1)
    ...
    if winner_index.numel() >= point_count:
        selected = winner_index[
            torch.topk(score[winner_index], point_count, sorted=True).indices
        ]
        fill_count = 0
    else:
        remaining = torch.nonzero(~winner, as_tuple=False).squeeze(1)
        needed = point_count - winner_index.numel()
        fill = remaining[
            torch.topk(score[remaining], needed, sorted=True).indices
        ]
        selected = torch.cat((winner_index, fill))
        fill_count = int(needed)
    result = RaLDPointPrediction(
        xyz_m=xyz[selected],
        coordinates_rae=coordinates[selected],
        probability=probability[selected],
        confidence=confidence[selected],
